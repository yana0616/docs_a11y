...
`<fieldset>`要素および`<legend>`要素において、**CSSのdisplayプロパティ（`display: flex`や`display: table`）は効きません**。  
その打開策として、`display: contents`を利用する（ないものとして扱う的な意味）方法がありますが、**IE11では効きません**。。。  
そのため、「入力項目名と入力欄が横並び」のようなよくあるレイアウトにおいて、`<fieldset>`要素および`<legend>`要素を使用してコーディングすることは、現状難しいと考えられます。  
（SPのように、「入力項目名の下に入力欄がある」ようなレイアウト構成であれば、使用可能。）  
【参考】
