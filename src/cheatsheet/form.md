...
::: danger グルーピングする際の注意（CSS的なところ）
`<fieldset>`要素および`<legend>`要素は、**`display: flex`や`display: table`といったdisplayプロパティが効きません**。  
ハック法もあります（`display: contents`を利用する）が、**IE11では効きません**。。。  
そのため、「入力項目名と入力欄が横並び」というPCでよくあるレイアウトにおいて、`<fieldset>`要素および`<legend>`要素を使用してコーディングすることは、現状難しいと考えられます。  
（SPのように、「入力項目名の下に入力欄がある」ようなレイアウト構成であれば使用可能。）  
グルーピングの必要がある場合は、role属性やaria-labelledby属性を利用したほうが良いかもしれません。
【参考】
* [『CSSが効かない！？fieldset要素でも簡単にテーブルレイアウトを実現できるdisplay: contents;』（ミツエーリンクス）](https://www.mitsue.co.jp/knowledge/blog/frontend/201806/28_0909.html)
:::
